...
router.get("/future-meals", async (req, res) => {
  try {
    console.log("hello");
    const row = await knex.raw("SELECT * FROM meals WHERE `date` > now()");
...
router.get("/all-meals", async (req, res) => {
  try {
    const meals = await knex.raw("SELECT * FROM meals ORDER BY ID");
...
router.get("/first-meal", async (req, res) => {
  try {
    const meal = await knex.raw("SELECT * FROM meals ORDER BY id ASC limit 1");
    if (meal) {
...
router.get("/last-meal", async (req, res) => {
  try {
    const meal = await knex.raw("SELECT * FROM meals ORDER BY id desc limit 1");
    if (meal) {
      res.status(200).json(meal[0][0]);
    } else {
      res.status(404).json("there are no meals");
